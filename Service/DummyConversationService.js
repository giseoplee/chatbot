...
DummyConversationService.Init = () => {
      ...
      config.dummyData.context.expires = moment().add('minutes', 60).valueOf();

      redis.context.set(config.dummyUserKey, JSON.stringify(config.dummyData.context)).then((result) => {
...
DummyConversationService.GetConversationResponse = (callback) => {
    ...
    redis.context.get(key).then((result) => {
          ...
          var data = redis.context.set(key, JSON.stringify(config.dummyData.context), () => {
